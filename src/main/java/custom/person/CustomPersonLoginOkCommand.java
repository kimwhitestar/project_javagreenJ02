...
public class CustomPersonLoginOkCommand implements CustomPersonInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		CustomPersonLoginVO vo = dao.searchCustomPersonLogin(id, shaPwd);
		//로그인 아이디,비밀번호로 회원조회가 안됬을 경우, 로그인 화면으로 이동
		if (null == vo) {
			request.setAttribute("msg", "customPersonLoginNo");
		...
		session.setAttribute("sCustomId", vo.getCustomId());//고객ID
    ...
    String todayYmd = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
		...
		request.setAttribute("msg", "customPersonLoginOk");
